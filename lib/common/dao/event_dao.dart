...
import 'package:github_flutter/db/provider/event/received_event_db_provider.dart';
import 'package:github_flutter/db/provider/event/user_event_db_provider.dart';
  ...
  static getEventDao(userName, {page = 0, bool needDb = false}) async {
    ...
    next() async {
      ...
      String url =
          Address.getEvent(userName) + Address.getPageParams("?", page);
      // String url =
      //     "https://api.github.com/users/yuefan6254/events?page=1&per_page=20";
      var res = await httpManager.netFetch(url, null, null, null);
      print('events 结果');
      print(res.toString());
      if (res != null && res.result) {
